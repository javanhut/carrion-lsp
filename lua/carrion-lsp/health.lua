...
function M.check()
  ...
  if installer.is_installed() then
    ...
    health.ok("carrion-lsp binary found: " .. (version or "unknown version"))
  else
    health.error("carrion-lsp binary not found", {
  ...
  if has_lspconfig then
    ...
    if configs.carrion_lsp then
      health.ok("carrion-lsp configuration loaded")
    else
      health.warn("carrion-lsp not yet configured (will be configured on first .crl file)")
    end
  else
    health.error("nvim-lspconfig not found", {
  ...
  if has_cmp then
    health.ok("nvim-cmp found (enhanced autocompletion enabled)")
    
    local has_cmp_lsp, _ = pcall(require, "cmp_nvim_lsp")
    if has_cmp_lsp then
      health.ok("cmp-nvim-lsp found (LSP completions enabled)")
    else
      health.warn("cmp-nvim-lsp not found", {
        "Install cmp-nvim-lsp for LSP completions: https://github.com/hrsh7th/cmp-nvim-lsp"
      })
    end
  else
    health.warn("nvim-cmp not found (basic autocompletion only)", {
  ...
  if ft == "carrion" then
    health.ok("Carrion filetype detection working")
  else
    health.error("Carrion filetype detection not working")
  end
  ...
  if lsp.is_running() then
    ...
    health.ok("carrion-lsp server running (ID: " .. client.id .. ")")
  else
    health.info("carrion-lsp server not currently running (will start when .crl file is opened)")
  end
  ...
  if vim.fn.executable("carrion-lsp") == 1 then
    ...
    health.ok("carrion-lsp found in PATH: " .. which_result)
  else
    health.warn("carrion-lsp not in PATH", {
  ...
  for _, dep in ipairs(optional_deps) do
    ...
    if has_dep then
      health.ok(dep[1] .. " found (enhanced features available)")
    else
      health.info(dep[1] .. " not found (optional)", {
